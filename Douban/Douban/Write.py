...
class Write():
    def __init__(self,item,path):
        ...
        self.path=path
        self.key=["电影效果","电影剧情"]
        self.value=[0.05,0.06]
        name=""
        self.detailkey=["特效,效果,特写,镜头,画面,渲染,视觉,感官,绚丽,3D,震撼","开场,故事,接下来,编剧,剧情,题材,讲述,告诉,气氛,渲染,故事线,视角,主题,理解,撼人心魄,感染观众,结尾,结局,最后,线索,叙事,节奏"]
    def run(self):
        tmp=[]
        tmppath =self.path+u'/电影'
        if not os.path.isdir(tmppath):
            os.mkdir(tmppath)
        # tmppath = tmppath+'/%s' %self.item['movie_keyword']
        # if not os.path.isdir(tmppath):
        #     os.mkdir(tmppath)
